...
import { ByComparator, EnsureArray, Expand, Props } from '../lib/types';

import { useComputed } from '../lib/hooks/use-computed';
import { useDisposables } from '../lib/hooks/use-disposables';
import { useEvent } from '../lib/hooks/use-event';
import { useId } from '../lib/hooks/use-id';
import { useIsoMorphicEffect } from '../lib/hooks/use-iso-morphic-effect';
import { useLatestValue } from '../lib/hooks/use-latest-value';
import { useOutsideClick } from '../lib/hooks/use-outside-click';
import { useResolveButtonType } from '../lib/hooks/use-resolve-button-type';
import { useSyncRefs } from '../lib/hooks/use-sync-refs';
import { useTreeWalker } from '../lib/hooks/use-tree-walker';
...
import { isDisabledReactIssue7711 } from '../lib/utils/bugs';
import { match } from '../lib/utils/match';
import { objectToFormEntries } from '../lib/utils/form';
import { sortByDomNode } from '../lib/utils/focus-management';

import { Hidden, Features as HiddenFeatures } from '../lib/hidden';
import { useOpenClosed, State, OpenClosedProvider } from '../lib/open-closed';

import { Keys } from '../lib/keyboard';
import { useControllable } from '../lib/hooks/use-controllable';
import { useWatch } from '../lib/hooks/use-watch';
import { useTrackedPointer } from '../lib/hooks/use-tracked-pointer';
...
function adjustOrderedState<T>(
	...
	adjustment: (
		options: StateDefinition<T>['options']
	) => StateDefinition<T>['options'] = (i) => i
) {
	...
	[ActionTypes.OpenCombobox](state) {
		...
		if (state.dataRef.current) {
			...
			let optionIdx = state.options.findIndex((option) =>
				isSelected(option.dataRef.current.value)
			);
...
let ComboboxActionsContext = createContext<{
	...
	registerOption(
		id: string,
		dataRef: ComboboxOptionDataRef<unknown>
	): () => void;
...
>(
	...
	let [value = multiple ? [] : undefined, theirOnChange] =
		useControllable<any>(controlledValue, controlledOnChange, defaultValue);
	...
	let optionsPropsRef = useRef<_Data['optionsPropsRef']['current']>({
		static: false,
		hold: false,
	});
	...
	type TActualValue = true extends typeof multiple
		? EnsureArray<TValue>[number]
		: TValue;
	...
	let isSelected: (value: TValue) => boolean = useCallback(
		(compareValue) =>
			match(data.mode, {
				[ValueMode.Multi]: () =>
					(value as EnsureArray<TValue>).some((option) =>
						compare(option, compareValue)
					),
				[ValueMode.Single]: () =>
					compare(value as TValue, compareValue),
			}),
		[value]
	);
	...
	let slot = useMemo<ComboboxRenderPropArg<unknown>>(
		() => ({
			...
			activeOption:
				data.activeOptionIndex === null
					? null
					: (data.options[data.activeOptionIndex].dataRef.current
							.value as TValue),
			value,
		}),
		[data, disabled, value]
		...
		<ComboboxActionsContext.Provider value={actions}>
			<ComboboxDataContext.Provider value={data}>
				...
				>
					{name != null &&
						value != null &&
						objectToFormEntries({ [name]: value }).map(
							([name, value], idx) => (
								<Hidden
...
>(props: ComboboxInputProps<TTag, TType>, ref: Ref<HTMLInputElement>) {
	...
	useWatch(
		([newState], [oldState]) => {
			if (
				newState === ComboboxState.Open &&
				oldState === ComboboxState.Closed
			) {
	...
	let composedChangeEvent =
		useRef<React.ChangeEvent<HTMLInputElement> | null>(null);
			...
			switch (event.key) {
				...
				case Keys.Escape:
					...
					if (
						...
						!data.optionsPropsRef.current.static
					) {
						event.stopPropagation();
					}
...
function ButtonFn<TTag extends ElementType = typeof DEFAULT_BUTTON_TAG>(
	...
	let internalId = useId();
	let { id = `headlessui-combobox-button-${internalId}`, ...theirProps } =
		props;
	let d = useDisposables();
...
function LabelFn<TTag extends ElementType = typeof DEFAULT_LABEL_TAG>(
	...
	let internalId = useId();
	let { id = `headlessui-combobox-label-${internalId}`, ...theirProps } =
		props;
...
function OptionsFn<TTag extends ElementType = typeof DEFAULT_OPTIONS_TAG>(
	...
	let ourProps = {
		...
		role: 'listbox',
		'aria-multiselectable':
			data.mode === ValueMode.Multi ? true : undefined,
		id,
		ref: optionsRef,
	};
...
let Options = forwardRefWithAs(
	OptionsFn
) as unknown as ComponentComboboxOptions;
